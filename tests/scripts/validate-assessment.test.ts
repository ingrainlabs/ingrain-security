 ...
 * on that shape.
 */
const SEEDED_SKELETON = `# Security assessment — Token refresh endpoint
...
Deno.test("skeleton: a freshly seeded file is lenient-valid and strictly invalid", async () => {
  const lenient = await validate("assessment-x.md", SEEDED_SKELETON, ["--lenient"]);
  assertEquals(lenient.code, 0, JSON.stringify(lenient.json.errors));

  // Strict must reject it: the sections are on the page but not yet filled in.
  const strict = await validate("assessment-x.md", SEEDED_SKELETON);
  assertEquals(strict.code, 1);
  assert(strict.json.error_count > 0);
  assert(
    strict.json.errors.every((e) => /is empty|missing required field/.test(e.message)),
    `expected only unfilled-field violations, got: ${JSON.stringify(strict.json.errors)}`,
  );
});
